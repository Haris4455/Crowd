...
def post_edit(request, post_id):
	...
	files_objs = []
	post = Post.objects.get(id=post_id)
	postform = NewPostForm(request.POST or None, instance=post)
	if postform.is_valid():
		tags_form = postform.cleaned_data.get('tags')
		files = request.FILES.getlist('content')
		tags_list = list(tags_form.split(','))
		...
		for file in files:
			file_instance = PostFileContent(file=file, user=request.user)
			file_instance.save()
			files_objs.append(file_instance)
